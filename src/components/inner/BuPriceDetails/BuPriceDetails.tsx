...
import styles from './BuPriceDetails.module.scss';
...
import {
    calculationOnOption,
    calculationTypeOption, calculationTypeOptions, ModalProps, NumberInput, style,
    StyledTableCell,
    StyledTableRow,
    UnitOption
...
const BuPriceDetails: React.FC<ModalProps> = ({ isOpen, onRequestClose, getbusinessUnitRowData, getBasePriceData, buPriceDetailsList }) => {
    ...
    React.useEffect(() => {
     ...
     //   showInfo("Inside useEffect")
        ...
        if (buPriceDetailsList && buPriceDetailsList.length && !isRowAdded) {
            const formattedData: any[] = buPriceDetailsList.map((item: any) => ({
                Id: item.Id || uuidv4(),
                PriceLabel: item.PriceLabel || '',
                PriceType: item.PriceType,
                Value: item.Value || '',
                CalculationOn: item.CalculationOn || '',
                CalculationType: (item.CalculationType === 0 ? "Increment" : "Decrement") || '',
    ...
    const handleRowInputChange = (index: any, fieldName: any, value: any) => {
        ...
        if (fieldName === 'PriceLabel') {
            switch (value) {
                ...
                case 'SGST':
                case 'CGST':
                    updatedRows[index]['PriceType'] = '1';
                    break;
                case 'Freight':
                case 'Base Price':
                case 'SUBT':
                    updatedRows[index]['PriceType'] = '0';
                    break;
                default:
                    updatedRows[index]['PriceType'] = ''; // Set to default or leave it empty if needed
            }
        }
        if (fieldName === 'CalculationType') {
            updatedRows[index]['CalculationType'] = parseInt(value, 10);
        }
        setrows(updatedRows);
    };
    ...
    const addRow = () => {
        ...
        const newRow = {
            'Id': newRowId,
            'PriceLabel': '', // Initialize other properties as needed
            'PriceType': '',
            'Value': '',
            'CalculationOn': '',
            'CalculationType': '',
        ...
        const rowData = getbusinessUnitRowData();
        setbusinessUnit(rowData)
    }
    ...
    const showTableRow = rows.map((rowData: any, index: any) => (
        <StyledTableRow className={styles.Row}>
            <StyledTableCell className={styles.BodyRow} component="th" scope="row">
                <FormControl
                ...
                >
                    <Select
                        ...
                        value={rowData['PriceLabel']}
                        displayEmpty={true}
                        onChange={(e) => handleRowInputChange(index, 'PriceLabel', e.target.value)}
                    >
                        <MenuItem value={""} selected={true} disabled={true}>
                            <Typography className="MenuItem">Select</Typography>
                        </MenuItem>
                        {priceLabelData.map((item: any) => (
                            <MenuItem className="SelectText" key={item.compId} value={item.compName}>
                                {item.compName}
                            </MenuItem>
                        ))}
                    </Select>
            ...
            <StyledTableCell className={styles.BodyRow} align="left">
                <FormControl className={styles.FormControl} size="small">
                    <Select
                        ...
                        id="priceType-select"
                        className={styles.Select}
                        value={rowData['PriceType']}
                        onChange={(e) => handleRowInputChange(index, 'PriceType', e.target.value)}
                    >
            ...
            <StyledTableCell className={styles.BodyRow} align="left">
                ...
                >
                    <Select
                        ...
                        className={styles.Select}
                        value={rowData['CalculationOn'] || 'none'}
                        onChange={(e) => handleRowInputChange(index, 'CalculationOn', e.target.value)}
                    >
                        <MenuItem value={"none"} selected={true} disabled={true}>
                            <Typography className="MenuItem">Select</Typography>
                        </MenuItem>
                        {calculationOnData.map((item: any) => (
                            <MenuItem className="SelectText" key={item.id} value={item.name}>
                                {item.name}
                            </MenuItem>
                        ))}
                    </Select>
            ...
            <StyledTableCell className={styles.BodyRow} align="left">
                ...
                >
                    <Select
                        ...
                        className={styles.Select}
                        value={rowData['CalculationType'] === 0 ? 0 : 1}
                        // @ts-ignore
                        onChange={(e) => handleRowInputChange(index, 'CalculationType', parseInt(e.target.value, 10))}
                    >
                        <MenuItem value={"none"} selected={true} disabled={true}>
                            <Typography className={styles.MenuItem}>Select</Typography>
                        </MenuItem>
                        {calculationTypeOptions.map((option) => (
                            <MenuItem className="SelectText" key={option.id} value={option.id}>
                                {option.name}
                            </MenuItem>
                        ))}
                    </Select>
            ...
            <StyledTableCell align="left" className={styles.BodyRow}>
                <Button className={styles.RemoveBtn} onClick={() => handleRemoveRow(index)}>
